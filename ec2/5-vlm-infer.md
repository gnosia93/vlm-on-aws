...
*멀티모달 모델의 GPU 사이징* 에서 가장 먼저 이해해야 할 점은, 병목이 모델 가중치가 아니라 비주얼 토큰 때문에 폭발하는 KV 캐시라는 사실입니다. 텍스트와 달리 이미지와 영상은 토큰으로 변환될 때 그 수가 매우 많아지는데, 특히 영상은 이미지보다 토큰 수가 수십 배에 달하기 때문에 실제 VRAM의 대부분이 여기서 소모됩니다. 따라서 사이징을 할 때는 총 VRAM을 모델 가중치, KV 캐시, 그리고 비전 인코더와 활성값·오버헤드라는 세 덩어리로 나누어 계산하는 것이 좋습니다.
