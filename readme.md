...
학습 데이터로 ** FineVideo **를 사용합니다. FineVideo는 Hugging Face가 공개한 대규모 영상 이해용 데이터셋으로, YouTube에 Creative Commons Attribution(CC-BY) 라이선스로 공개된 약 43,000개의 실사 영상(평균 4.7분, 총 약 3,425시간, 122개 카테고리)을 YouTube-Commons로부터 수집해 구성했습니다.
...
라이선스 측면에서도 모든 영상이 CC-BY 기반이라 워크샵 진행과 결과물 공유에 대한 부담이 적으며, 각 데이터에 원본 출처(provenance)와 채널 정보가 포함되어 있어 저작자 표시(attribution) 요건을 충족하기 용이합니다. 데이터셋은 FineVideo Space에서 브라우저로 직접 탐색해볼 수 있습니다.
